...
Suppose that the problem data is contained in a file called `input.txt` located in the same directory as the JavaScript file, for example:
...
Note the use of the Javacript regular expression `/\n/`).  (In Javascript regular expressions to not start life as strings, as they do in R.  Instead they are a special type of object, delimited by the forward-slash: `/`[^1]. 

[^1]: For more on regular expressions in JavaScript, see [Chapter 8](https://eloquentjavascript.net/09_regexp.html) of the *Eloquent JavaScript* textbook.)
...
(Math.max() takes as arguments all of the numbers of whihc we wish to find the maximum.  Note the use of the three-dot notation notation `...` to convert the single array into separate arguments[^2].

[^2]: For a review of the three-dot notation, see the section [Rest Parameters](https://eloquentjavascript.net/04_data.html#h_hX9DkIBp9y) in *Eloquent JavaScript*. )
...
A complete solution to the problem may be found [here](main.js), or the reader may view it below:
