...
To investigate the relationship of age, time and sex all at once, it might be preferable to include `sex` as an aesthetic property in the graph.  The over-plotting could als be addressed by a different sort of glyph:  density contours, for example, rather than points.  Below is a non-interactive graph of the data:
...
Since the density contours of the two sexes overlay each other somewhat, it may be best to facet by `sex` rather than making it as aesthetic property of our glyphs:
...
The version of Plot porvided with Quarto often lags well behind the latest version as [documented](```{ojs}
Plot = import("https://esm.run/@observablehq/plot")
```) by Observables.  In order to keep your Quarto project current with what works in your Observables notebooks, it is advisable to import the latest version of Plot, like this:
