...
We need to join with the Teams data.  We also plan to add infromation for tool tips.  As a challenge, I would like to include the debut date of th eplayer in this information.  This requires extracting just the date from the `debut` variable, which has datetime values.  Sadly, Arquero cannot handle certain functions in its table computations, so, following the advice in a helpful Arquero error message, I add a function:

```{ojs}
aq.addFunction('toDateString', date => date.toDateString());
```

Now for the join and the mutations:
...
df = hrLeaders.
  ...
  // make tool-tip info:
  .derive(
    {debutDate: d => op.toDateString(d.debut)}
  )
  .derive(
